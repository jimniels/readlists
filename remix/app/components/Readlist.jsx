...
export default function Readlist({ readlist }) {
  ...
  const handleDeleteReadlist = (e) => {
    ...
    if (window.confirm(msg)) {
      console.log(
        "DELETE readlist from local storage and then let the action continue"
      );
  ...
  const isLoadingEpub = false;
  const disabledButtons = isLoadingEpub || readlist.articles.length === 0;

  return (
    <form method="POST" action="/?index" class="readlist wrapper">
      <header class="readlist-header">
        ...
        <form method="POST" action="/?index"></form>
